...
class Upload
  ...
  def to_json(*a)
    [{
      "current"       => {"id" => @id, "type" => @type, "file_name" => @file_name, "file_name" => "http://localhost:9292/upload/2/download", "userid" => @userid, "upload_datetime" => @upload_datetime, "overallScore" => @overallScore, "numOfRatings" => @numOfRatings, "title" => "My lovely cat", "description" => "Nice cat - yeehhaaa", "tags" => ""}, "previous" => {"id" => "1", "type" => "image", "rating" => 4, "file_name" => "public/img/cat2.jpg"}, "next" => {"id" => "3", "type" => "image", "rating" => 3, "file_name" => "public/img/cat3.jpg"}
      }].to_json(*a)
  end
